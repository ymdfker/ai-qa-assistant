...
const DB_PATH = path_1.default.join(electron_1.app.getPath('userData'), 'aiqa.db');
...
const insertModel = db.prepare(`INSERT OR IGNORE INTO model_configs (model_name,display_name,api_endpoint,thinking_levels,file_support,is_preset,is_enabled)
   VALUES (?,?,?,?,?,1,1)`);
...
const stmts = {
    ...
    addCustomModel: db.prepare(`INSERT INTO model_configs (model_name,display_name,api_endpoint,is_preset)
     VALUES (?,?,?,0)`),
    ...
    // Sessions
    createSession: db.prepare(`INSERT INTO sessions (title, model_name) VALUES (?, ?)`),
