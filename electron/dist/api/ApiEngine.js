...
exports.setupApiIPC = setupApiIPC;
...
function setupApiIPC() {
    ...
    electron_1.ipcMain.on('api:summarize', (event, job) => {
        const { modelName, sessionId, messages: msgs } = job;
        const models = Database_1.stmts.getModels.all();
        ...
        const req = transport.request({
            ...
            res.on('end', () => { try {
                event.sender.send('api:summarize-done', { sessionId, title: result.trim().slice(0, 30) });
            }
            catch { } });
        });
        req.on('error', () => { try {
            event.sender.send('api:summarize-done', { sessionId, title: '' });
        }
        catch { } });
        req.write(body);
        req.end();
    });
    electron_1.ipcMain.on('api:send', (event, job) => {
        const { modelName, sessionId, messages: msgs } = job;
        ...
        const req = transport.request({
            ...
            res.on('data', (chunk) => {
                chunkCount++;
                if (chunkCount === 1)
                    console.log(`[api:${modelName}] first chunk after ${Date.now() - startTime}ms`);
    ...
    electron_1.ipcMain.on('api:cancel', () => {
